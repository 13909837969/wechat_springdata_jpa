...
function login(){
	...
	$.ajax({
		type:"post",
		url:"weixin/users_login",
		data:{"email":email,"openid":openid},
		success:function(msg){
			if("1"==msg){
				openPopup('您输入的邮箱在本系统中不存在（请联系管理员申请）','提示',undefined,true,undefined,'error','cn');
			}else if("2"==msg){
				openPopup('绑定失败，您输入的邮箱已被人绑定，如果疑问','提示',undefined,true,undefined,'error','cn');
			}else{
				//页面关闭
				WeixinJSBridge.call('closeWindow');
