...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
	 ...
	 <a href="javascript:;" onclick="loginusers_add('添加用户','pages/admin/loginusers/loginusers_add.jsp','','510')" class="btn btn-primary radius">
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			  ...
			  <c:forEach items="${loginUsersList }" var="loginUsers">	
				<tr class="text-c">
					<td><input type="checkbox" value="" name=""></td>
					<td>${loginUsers.id }</td>
					<td class="text-l">
					       <u style="cursor:pointer" class="text-primary" 
					       onClick="article_edit('查看','article-zhang.html','10001')" title="查看">${loginUsers.username}</u></td>
					
					<td>${loginUsers.lastIp }</td>
					<td>${loginUsers.lastCurrtDate }</td>
					<td>21212</td>
					<td class="td-status">
					<c:if test="${loginUsers.status==0 }">
						<span class="label label-success radius">
						正常
						</span>
					</c:if>
					<c:if test="${loginUsers.status==1 }">
						<span  style="color:red">
						失效
						</span>
					</c:if>
					<c:if test="${loginUsers.status==2 }">
					...
					<a style="text-decoration:none" class="ml-5" onClick="article_del(this,'${loginUsers.id }')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
				</c:forEach>
			</tbody>
		</table>
...
/*用户-添加*/
function loginusers_add(title,url,w,h){
